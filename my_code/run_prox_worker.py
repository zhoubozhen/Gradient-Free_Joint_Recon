...
def _read_proc_status_kb(key: str):
    # /proc/self/status: e.g. "VmRSS:   123456 kB"
    try:
...
def _mem_snapshot_gb():
    # RSS via psutil if available; fallback to /proc/self/status
    rss_kb = None
    swap_kb = None
    try:
        ...
        # per-process swap is not reliably available in psutil; read from /proc instead
    except Exception:
...
def _gpu_snapshot_nvml():
    # returns list of dicts: [{"idx":0,"util":..,"used_gb":..,"total_gb":..}, ...]
    try:
        ...
        for i in range(n):
            ...
            out.append({"idx": i, "util": int(util), "used_gb": used_gb, "total_gb": total_gb})
        pynvml.nvmlShutdown()
        return out
    except Exception:
        return None
...
def _gpu_snapshot_smi():
    # fallback: nvidia-smi --query-gpu=utilization.gpu,memory.used,memory.total --format=csv,noheader,nounits
    try:
        cmd = [
            "nvidia-smi",
        ...
        for i, line in enumerate(lines):
            ...
            if len(parts) >= 3:
                ...
                out.append({
                    "idx": i,
                    "util": util,
                    "used_gb": used_mib * 1024.0 * 1024.0 / 1e9,
                    "total_gb": total_mib * 1024.0 * 1024.0 / 1e9,
                })
        return out
    except Exception:
        return None
...
def main():
    ...
    # start heartbeat early (before cupy import is fine)
    hb_sec = int(os.environ.get("PROX_HEARTBEAT_SEC", "10")) # 设置心跳时间
    hb_stop = start_heartbeat(tag="HEARTBEAT", interval_sec=hb_sec)

    try:
        # import cupy AFTER CUDA_VISIBLE_DEVICES is set
        import cupy as cp
        dev = cp.cuda.Device()
