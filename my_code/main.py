...
def _set_env_before_cupy(cfg: dict):
    ...
    # non-MPI: 不强制覆盖 CUDA_VISIBLE_DEVICES（让 run.sh/condor 来决定）
    # 但如果 config 里给了 gpu（可选），就覆盖
...
def main():
    ...
    cli_override = {}
    for k in ["pressure", "recon_opt", "iter", "maxfun", "lip", "reg", "stride", "start", "prox_mode"]:
        v = getattr(args, k)
        if v is not None:
            cli_override[k] = v
    if args.mpi:
    ...
    # 让 new_v2/src 优先被 import（不靠手动 sys.path.append 到处写）
    repo_root = _get_repo_root_from_file()
    new_v2_src = os.path.join(repo_root, "new_v2", "src")
    if new_v2_src not in sys.path:
        sys.path.insert(0, new_v2_src)

    # 工具（你已有 gfjr_utils.py）
    from new_v2.src.gfjr_utils import (
    ...
    reg_param = float(cfg.get("reg", 0.01))
    lip = float(cfg.get("lip", 5.0))
    num_iter = int(cfg.get("iter", 50))
    ...
    with StageTimer("build model"):
        if recon_opt == 0:
            acnhp = sio.loadmat(os.path.join(nhp_directory, "acoustic/nhp_1p1l_vifov.mat"))["annhp"]
            if DEBUG_SMALL:
            ...
            acnhp = sio.loadmat(os.path.join(nhp_directory, "acoustic/nhp_1p3l_vifov.mat"))["annhp"]
            if DEBUG_SMALL:
                acnhp = apply_downsample_3d(acnhp, ix, iy, iz)
            model = TranPACTModel(
                space_order=so, medium_geometry=acnhp,
