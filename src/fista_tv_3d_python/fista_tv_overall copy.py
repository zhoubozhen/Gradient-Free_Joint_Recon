...
# ==========================================================
# NEW: divergence early-stop (dcost + rel)  <-- 你要加的这套
# ==========================================================
...
def fista_tv(pmeas, Nx, Ny, Nz,
    ...
    if ENABLE_TV:
        proximal_L = call_proximal(prox_mode)
        _log(f"proximal selected: mode={prox_mode} -> {proximal_L.__module__}.{proximal_L.__name__}", mpi_rank)
    ...
    for iter in range(start_iter, niter_max + 1):
        ...
        # does not change algorithm, only decides to stop earlier when already converged
