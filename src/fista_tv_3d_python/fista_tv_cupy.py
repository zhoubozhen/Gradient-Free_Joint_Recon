...
def fista_tv(pmeas, Nx, Ny, Nz, forward_op, adjoint_op, roi, \
        reg_param, lip, positive_constraint, niter_max, cost_min, saving_dir, \
             start_iter=1, init_guess=None, mpi_rank=0, init_tp=1, old_cost=None):
  ...
  for iter in range(start_iter, niter_max + 1):
    ...
    p0 = proximal_L(p0y -  (2/lip)*dp0, Nx, Ny, Nz, 2*reg_param/lip, positive_constraint, 50)
    proxi_end = time.time()
    if mpi_rank==0:
    ...
    tp = tn
    if mpi_rank==0:
      fname = os.path.join(saving_dir, 'fn_checkpoint')
      np.save(fname,tn)
    p0p = p0
