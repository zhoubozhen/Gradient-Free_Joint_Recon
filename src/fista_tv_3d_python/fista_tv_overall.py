...
REL_THR_DEFAULT = 5e-2         # stop if rel < thr ...
...
# ------------------------------
# Safe dynamic proximal loader
# ------------------------------
def call_proximal(prox_mode=1):
    # normalize
    if isinstance(prox_mode, (tuple, list)) and len(prox_mode) == 1:
        prox_mode = prox_mode[0]
    prox_mode = int(prox_mode)
    if prox_mode == 0:
        name = "proximal_L"
    elif prox_mode == 1:
...
def _infer_v2_root():
    """
    Infer V2_tesla root from this file location:
      .../V2_tesla/src/fista_tv_3d_python/fista_tv_overall.py
    """
    here = os.path.abspath(os.path.dirname(__file__))
    # here = .../src/fista_tv_3d_python
    v2_root = os.path.abspath(os.path.join(here, "..", ".."))  # -> .../V2_tesla
    return v2_root

def _call_prox_subprocess(p0_in, Nx, Ny, Nz, reg_eff, positive_constraint, prox_iter,
                          saving_dir, iter_idx, mpi_rank):
    """
    ...
    if not prox_cuda:
        raise RuntimeError(
            "PROX_CUDA_VISIBLE_DEVICES is NOT set.\n"
            "Your wrapper.sh must export the prox GPU UUID."
        )

    # 🔒 HARD BIND TO PHYSICAL GPU UUID
    env["CUDA_VISIBLE_DEVICES"]   = prox_cuda
    env["NVIDIA_VISIBLE_DEVICES"] = prox_nv if prox_nv else prox_cuda

    # OpenACC safety: only one visible device → index 0
    ...
    # --- shape safety: keep everything as 3D (Nx,Ny,Nz) inside FISTA ---
...
def fista_tv(pmeas, Nx, Ny, Nz,
    ...
    _log(
        ...
        f"prox_iter={prox_iter} prox_mode={prox_mode} "
    ...
    if ENABLE_TV:
        ...
        if pmode == 2:
            _log("proximal selected: prox_mode==2 -> subprocess worker (GPU split)", mpi_rank)
        else:
            proximal_L = call_proximal(pmode)
            _log(f"proximal selected: mode={pmode} -> {proximal_L.__module__}.{proximal_L.__name__}", mpi_rank)
