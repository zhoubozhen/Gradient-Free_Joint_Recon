...
class ReducedCostFunction:
    ...
    def __init__(self, p0_est, measured_pressure, fn, opt_param, wave_solver,
        ...
        if use_check_runtime and os.path.exists(self.saving_dir + 'datafid_record.DAT'):
            ...
            self.obj_record = np.fromfile(
                self.saving_dir + "datafid_record.DAT", dtype=np.float32
            ).tolist()
            self.iter_record = np.fromfile(
                self.saving_dir + "iter_record.DAT", dtype=np.float32
            ).tolist()
            self.c0_record = np.fromfile(
                self.saving_dir + "c0_record.DAT", dtype=np.float32
            ).reshape(len(self.obj_record), -1).tolist()
        else:
    ...
    def medium_set(self, sos_local, use_static=True, use_downsample=False):
        ...
        if self.medium_mode == 'mpml':
            ...
            if use_static:
                self.solver.model.initialize_medium(
                    fn_tmp, use_static=True, use_downsample=use_downsample
                )
            else:
                self.solver.model.initialize_medium(
                    fn_tmp, use_static=False, use_downsample=use_downsample
                )
        else:
            raise ValueError(
                f"Unknown medium mode: {self.medium_mode}. Supported modes are 'mpml' and 'aubry'."
            )
    ...
    def __call__(self, sos_local):
        ...
        if self.cur_iter < iter_ind:
            if self.cur_iter == iter_ind - 1:
                ...
                self.p0_est_global = np.fromfile(
                    target, dtype=np.float32
                ).reshape(self.p0_est.shape)
...
class GFJRSolver:
    ...
    """
    def __init__(self, solver, measured_pressure, fn, forward=None, adjoint=None,
                 opt_param=None, init_guess=None, use_static=True, use_downsample=False, **param):
        self.use_static = use_static
        self.use_downsample = use_downsample
        self.solver = solver
        ...
        self.cost_function = ReducedCostFunction(
            p0_est=self.p0_est,
            measured_pressure=self.data,
            fn=self.fn,
            opt_param=self.opt_para,
            ...
            **{k: v for k, v in param.items() if k not in ('rank', 'comm')}
        )
